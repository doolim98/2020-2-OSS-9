...
def change_pixel_index_to_button_index(x, y):
    if(x >= 230 and x <= 280 and y <= 300 and y >= 250):
        return 5;
...
def reloadbutton():
    goto(250, 300)
    color('black')
    write("Reload", move=True, align="center", font=("Arial", 15, "bold"))
    ...
    if(result == 5):
        ...
        if(board_show[array_y][array_x] is None):
            board_tofill[array_y][array_x] = int(
                numinput("num input", "plz enter an integer between 1 and 9", None, minval=1, maxval=9))
        sudoku_load()
        draw()
